...
class Login
{
    ...
    private $user_email = "";
	   /**
     * @var string $avatar The user's mail
     */
    private $avatar = "";
    /**
    ...
    public function __construct()
    {
        ...
        // if user has an active session on the server
            ...
            // user try to change his avatar
			}elseif (isset($_POST["user_edit_submit_avatar"])) {
                // function below uses use $_SESSION['user_id'] et $_SESSION['user_email']
                $this->editUserAvatar($_POST['avatar']);}
	...
	private function getAvatar($avatar)
    {
        ...
        if ($this->databaseConnection()) {
            // database query, getting all the info of the selected user
            $query_avatar = $this->db_connection->prepare('SELECT * FROM users WHERE user_name = :user_name');
			$query_avatar->bindValue(':avatar', $avatar, PDO::PARAM_STR);
            $query_user->execute();
            // get result row (as an object)
            return $query_user->fetchObject();
        } else {
            ...
            if ($hash == hash('sha256', $user_id . ':' . $token . COOKIE_SECRET_KEY) && !empty($token)) {
                ...
                if ($this->databaseConnection()) {
                    ...
                    if (isset($result_row->user_id)) {
						...
						$_SESSION['avatar'] = $result_row->avatar;
	...
	/**
     * Edit the user's avatar, provided in the editing form
     */
    ...
    {	
        ...
        if ($this->databaseConnection()) {
            // check if new email already exists
            $query_user = $this->db_connection->prepare('SELECT * FROM users WHERE user_email = $email');
            $query_user->bindValue(':avatar', $avatar, PDO::PARAM_STR);
            $query_user->execute();
            // get result row (as an object)
            $result_row = $query_user->fetchObject();

                // write users new data into database
                $query_edit_user_avatar = $this->db_connection->prepare('UPDATE users SET avatar = :avatar WHERE user_id = :user_id');
                $query_edit_user_avatar->bindValue(':avatar', $avatar, PDO::PARAM_STR);
                $query_edit_user_avatar->bindValue(':user_id', $_SESSION['user_id'], PDO::PARAM_INT);
                $query_edit_user_avatar->execute();

                if ($query_edit_user_avatar->rowCount()) {
                    $_SESSION['avatar'] = $avatar;
                    $this->messages[] = "Editing avatar successfull!";
                } else {
                    $this->errors[] = "Editing avatar failed!";
                }
		}
    }
